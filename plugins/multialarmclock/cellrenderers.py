...
class CellRendererDays(Gtk.CellRendererText):
    ...
    def do_start_editing(self, event, treeview, path, background_area, cell_area, flags):
        ...
        # position the popup below the edited cell (and try hard to keep the popup
        ...
        # save the path so we can return it in _done, and we aren't using dialog
        # so we can't block....
        self._path = path
    ...
    def _key_pressed(self, view, event):
        ...
        if not event or event.type == Gdk.EventType.KEY_PRESS and Gdk.keyval_name(
                event.keyval) == 'Return':
            self._done()
            return True
